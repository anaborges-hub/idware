...
export default function LiveMonitor() {
    ...
    return (
        <div className="relative w-full max-w-4xl mx-auto">
            {/* Dashboard Container */}
            <div className="bg-[#0b1221] border border-primary/30 rounded-lg overflow-hidden shadow-[0_0_50px_-12px_rgba(245,156,0,0.3)]">
                {/* Top Bar */}
                ...
                {/* Main Dashboard Grid */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 bg-black/20  h-auto lg:h-[400px]">
                    ...
                    <div className="lg:col-span-5 bg-black/40 flex flex-col h-[400px] lg:h-auto overflow-hidden">            <div className="p-3 border-b border-white/5 bg-white/5 flex justify-between items-center">
                        ...
                        <div className="flex-1 overflow-y-auto p-0 custom-scrollbar" ref={scrollRef}>
                            <div className="flex flex-col divide-y divide-white/5">
                                {logs.map((log) => (
                                    <div key={log.id} className={`p-3 hover:bg-white/5 transition-colors border-l-2 ${log.level === 'WARN' ? 'border-yellow-500' :
                                            log.level === 'SUCCESS' ? 'border-green-500' : 'border-primary'
                                        }`}>
                                        <div className="flex justify-between items-start mb-1">
                                            <span className={`text-[10px] font-bold font-mono ${log.level === 'WARN' ? 'text-yellow-500' :
